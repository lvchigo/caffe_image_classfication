...
//add by chigo 2015.11.11
double predict_probability_muti(const struct model *model_, const struct feature_node *x, double* prob_estimates, std::vector< std::pair< int, double > > &res)
{
	res.clear();
	if(check_probability_model(model_))
	{
		...
		if(nr_class==2) // for binary classification
		{
			...
			for(i=0; i<nr_class; i++)
			{
				res.push_back( std::make_pair( model_->label[i], prob_estimates[i] ) );
			}
		}
		else
		{

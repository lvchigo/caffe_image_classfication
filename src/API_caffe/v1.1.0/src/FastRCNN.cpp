...
FastRCNNCls::FastRCNNCls() {
	...
	if(boxes_.size()>1){ boxes_.clear();}
	if(img_input_.size()>1){ img_input_.clear();}
	if(roi_input_.size()>1){ roi_input_.clear();}
	if(img_shape_.size()>1){ img_shape_.clear();}
	if(roi_shape_.size()>1){ roi_shape_.clear();}
	//roi_input_ = NULL;
	//img_input_ = NULL;
//	box_model = NULL;
}
...
void FastRCNNCls::set_boxes( vector< pair<float, Vec4i> > &inBox ) {
	
	box_num_ = inBox.size();
	if (box_num_ < 1 ) cerr << "Error FastRCNNCls::set_boxes!" <<" Its size:"<<box_num_<<endl;

	boxes_.clear();
	boxes_.resize(box_num_);	

	for (int i = 0; i < box_num_; i++) {
		boxes_[i].push_back(inBox[i].second[0]);
		boxes_[i].push_back(inBox[i].second[1]);
		boxes_[i].push_back(inBox[i].second[2]);
		boxes_[i].push_back(inBox[i].second[3]);
	}

	//printf("boxes_.size():%d\n",boxes_.size());
}
...
// img must be a BGR image
	const float SCALES = 480;//320,480,600
	const float MAX_SIZE = 512;//512,1000
	...
	if(img_input_.size()>1){ img_input_.clear();}
	img_input_.reserve(imgout.cols * imgout.rows * imgout.channels());
	for (int j=0; j<imgout.channels(); j++) {
		for (int i=0; i<imgout.cols*imgout.rows; i++) {
			img_input_.push_back(((float*)imgout.data)[i*3]+j); 
		}
	} 
...
void FastRCNNCls::project_im_rois() {
	...
	if(roi_input_.size()>1){ roi_input_.clear();}
	roi_input_.reserve(roi_data_len_);
	for (int i=0; i<box_num_; i++) {
		roi_input_.push_back(0); 
		for (int j=1; j<=4; j++) 
			roi_input_.push_back(boxes_[i][j-1] * scale_factor_);
	} 
...
void FastRCNNCls::do_forward() {
	...
	// data change
	float* img_input = new float[img_input_.size()];
	float* roi_input = new float[roi_input_.size()];
	for (int i=0; i<img_input_.size(); i++) {
		img_input[i] = img_input_[i];
	} 
	for (int i=0; i<roi_input_.size(); i++) {
		roi_input[i] = roi_input_[i];
	} 

	// import the image data and roi data
	input_blobs[0]->Reshape(img_shape_);
	input_blobs[1]->Reshape(roi_shape_);
	memcpy(input_blobs[0]->mutable_cpu_data(), img_input, sizeof(float) * input_blobs[0]->count());
	memcpy(input_blobs[1]->mutable_cpu_data(), roi_input, sizeof(float) * input_blobs[1]->count());

	// caffe net forward and get the output data
	output_blobs = caffe_test_net_->ForwardPrefilled();

	if(img_input != NULL){ delete[] img_input; img_input = NULL;}
	if(roi_input != NULL){ delete[] roi_input; roi_input = NULL;}
	if(img_input_.size()>1){ img_input_.clear();}
	if(roi_input_.size()>1){ roi_input_.clear();}
}

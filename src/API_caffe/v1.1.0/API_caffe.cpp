...
{	
	...
	vecInputImg.clear();
	int loadImageNum = 1;//7;	//load image num:1-all image;2-center image;3~11-3*3 block
...
int API_CAFFE::DL_FileList2FRCNNLabel( 
	const vector<Mat_<Vec3f> > 						img_dl, 
	const vector< pair<float, Vec4i> >				img_box, 
	const UInt64 									ImageID,
	const long 										labelNum, 
...
{	
	...
	/*****************************load source*****************************/
	Blob<float> image_blob( vecInputImg.size(), CHANNEL, BLOB_HEIGHT, BLOB_WIDTH ); // input 
	if ( ReadImageToBlob( vecInputImg, 1, image_blob  ) )
	{
		cout<< "ReadImageToBlob Err!!" << endl;
		return 1;
	}

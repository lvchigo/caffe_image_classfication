...

{
	...
	/***********************************Load 71class File**********************************/
	net_dl.reset(new Net<float>(DL_DeployFile, caffe::TEST));
...
{	
	...
	vecInputImg.clear();
	vecInputImg.assign( img_dl.begin(), img_dl.begin()+LOAD_IMAGE_NUM );
	...
	shared_ptr<Blob<float> > feature_blob;

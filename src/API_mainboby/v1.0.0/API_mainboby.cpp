...
/*			nRet = api_commen.Normal_MinMax(imgFeat[i],normIn6ClassFeat);	//NormDLFeat FOR in5class
			...
			vecIn6ClassFeat.push_back(normIn73ClassFeat);	// FOR in6class
		}
	}
	...
	vector< pair< string, float > >	&Res)				//[Out]:Res:In37Class/ads6Class/imgquality3class
{
	...
	/************************Predict In37Class*****************************/
	{
		/************************SVM 90class Predict*****************************/	
		tmpRes.clear();
		nRet = api_libsvm_90class.Predict( vecIn73ClassFeat, tmpRes );	//PCA_FEAT FOR 72Class
		if (nRet != 0)
		{	
		...
		nRet = Merge90classLabel( tmpRes, ResIn37Class, tmpResIn37Class );
		if (nRet != 0)
		{	
	...
	const char* 					layerName)			//[In]:Layer Name by Extract
{
	...
	for(i=0;i<HypotheseBox.size();i++)
	{
		...
		IplImage *ImgResize = cvCreateImage(cvSize(256, 256), MutiROI->depth, MutiROI->nChannels);
		cvResize( MutiROI, ImgResize );
	...
	vector< pair< pair< string, Vec4i >, float > >	&Res)			//[In]:Layer Name by Extract
{
	...
	/************************Merge Res*****************************/
	nRet = Merge_Predict_Check( MergeRes, Res);
...
int API_MAINBOBY::Merge_Predict_Check(
	vector< pair< pair< string, Vec4i >, float > > 	inImgLabel,
	vector< pair< pair< string, Vec4i >, float > > 	&outImgLabel)
{
	if ( inImgLabel.size() < 1 ) 
	...
	outImgLabel.clear();
	int i, tmpSize, bin_only_other, nRet = 0;
	int topN = 5;
	vector< pair< pair< string, Vec4i >, float > >	MergeRes;
	...
	//filter only other.other.other
	bin_only_other = 1;
	for ( i=0;i<inImgLabel.size();i++ )
	{
		if ( inImgLabel[i].first.first != "other.other.other" )
		{
			bin_only_other = 0;
			break;
		}
	...
	//only write first other.other.other
	if ( bin_only_other == 1 )
		filterInfo[inImgLabel[0].first.first] = make_pair(inImgLabel[0].second,inImgLabel[0].first.second);
	else if ( bin_only_other == 0 )
	{
		for ( i=inImgLabel.size()-1;i>=0;i-- )
		{
			if ( ( inImgLabel.size()==1 ) || 
				 ( ( inImgLabel.size()>1 ) && ( inImgLabel[i].first.first != "other.other.other" ) ) )
			{
				filterInfo[inImgLabel[i].first.first] = make_pair(inImgLabel[i].second,inImgLabel[i].first.second);
			}
		}
...
int API_MAINBOBY::Merge90classLabel(
...
/*	//source
	...
	const float T_Label[90] = {		0.91,0.91,0.85,0.7, 0.96,0.8, 0.7, 0.91,0.94,0.8,	
			       					...
			       					0.9, 0.81,0.9, 0.9, 0.7, 0.76,0.9, 0.8, 0.9, 0.7,	//flower
			       					0.91,0.7, 0.8, 0.9, 0.7, 0.8, 0.5, 0.9, 0.7, 0.7,	
			        				0.91,0.8, 0.8, 0.9, 0.9, 0.7, 0.8, 0.9, 0.8, 0.85,	//shoe,text,people.*
...
*/	
	...
	for ( i=0;i<inImgLabel.size();i++ )
	{
		...
		if ( i == 0 )
		{
			...
			LabelInfo.push_back( std::make_pair( dic_90Class[tmpLabel], score ) );

...
int test_mobile( char *szQueryList, char* KeyFilePath, char* svPath, float MutiLabel_T )
{
	char tPath[256];
	...
	while(EOF != fscanf(fpListFile, "%s", loadImgPath))
	{
		...
		/************************change img format*****************************/
		uchar* bgr = api_commen.ipl2uchar( imgResize );

		/***********************************Predict**********************************/
		int rect[4] = {0};
		run.start();
		nRet = api_fast_segment.do_fast_segment( bgr, strImageID, imgResize->width, imgResize->height, imgResize->nChannels, rect );
		if (nRet!=0)
		{
		...
		/************************save img data*****************************/
		cvRectangle( imgResize, cvPoint(rect[0], rect[1]), cvPoint(rect[2], rect[3]), CV_RGB(255,0,0), 2, 8, 0);
		sprintf( savePath, "res/color/%s.jpg", strImageID.c_str() );
...
/*	if (argc == 6 && strcmp(argv[1],"-test") == 0) {
	...
	if (argc == 6 && strcmp(argv[1],"-test_mobile") == 0) {
		ret = test_mobile( argv[2], argv[3], argv[4], atof(argv[5]) );
	}
	else
	{
		cout << "usage:\n" << endl;
		cout << "\tDemo_fast_segment -test loadImagePath keyfile svPath MutiLabel_T\n" << endl;
		cout << "\tDemo_fast_segment -test_mobile loadImagePath keyfile svPath MutiLabel_T\n" << endl;
		return ret;
	}
	return ret;
}

...
Mat CmSaliencyRC::GetLC(const Mat &img3f)
{
	Mat img;
...
Mat CmSaliencyRC::GetHC(const Mat &img3f) 
{
	// Quantize colors and
	int colorNums[3] = {12, 12, 12};
	Mat idx1i, binColor3f, colorNums1i, _colorSal;
	Quantize(img3f, idx1i, binColor3f, colorNums1i, colorNums);
...
void CmSaliencyRC::GetHC(const Mat &binColor3f, const Mat &colorNums1i, Mat &_colorSal)
{
	Mat weight1f;
...
Mat CmSaliencyRC::GetRC(const Mat &img3f, string iid)
{
	return GetRC(img3f, iid, 0.4, 50, 200, 0.5);
}

Mat CmSaliencyRC::GetRC(const Mat &img3f, string iid, double sigmaDist, double segK, int segMinSize, double segSigma)
{
	...
	//char szImgPath[1024];
	//sprintf(szImgPath, "res/imgLab3f/%s.jpg", iid.c_str() );
	//imwrite( szImgPath, imgLab3f );
	//sprintf(szImgPath, "res/regIdx1i/%s.jpg", iid.c_str() );
	//imwrite( szImgPath, regIdx1i );
	//printf( "GetRC:regNum=%d\n", regNum);
	
	return GetRC(img3f, iid, regIdx1i, regNum, sigmaDist);
}

Mat CmSaliencyRC::GetRC(const Mat &img3f, string iid, const Mat &regIdx1i, int regNum, double sigmaDist)
{
	Mat colorIdx1i, regSal1v, tmp, color3fv;
	int colorNums[3] = {12, 12, 12};
	int QuatizeNum = Quantize(img3f, colorIdx1i, color3fv, tmp, colorNums);
	if (QuatizeNum == 2){
		printf("QuatizeNum == 2\n");
		Mat sal;
...
int CmSaliencyRC::Quantize(const Mat& img3f, Mat &idx1i, Mat &_color3f, Mat &_colorNum, int clrNums[3], double ratio)
{
...
void CmSaliencyRC::BuildRegions(const Mat& regIdx1i, vector<Region> &regs, const Mat &colorIdx1i, int colorNum)
{
...
void CmSaliencyRC::RegionContrast(const vector<Region> &regs, const Mat &color3fv, Mat& regSal1d, double sigmaDist)
{	
...
Mat CmSaliencyRC::GetBorderReg(const Mat &idx1i, int regNum, double ratio, double thr)
{
	// Variance of x and y
...
void CmSaliencyRC::SmoothSaliency(Mat &sal1f, float delta, const vector<vector<pair<float, int>>> &similar)
{
	Mat colorNum1i = Mat::ones(sal1f.size(), CV_32SC1);
	SmoothSaliency(colorNum1i, sal1f, delta, similar);
}

void CmSaliencyRC::SmoothSaliency(const Mat &colorNum1i, Mat &sal1f, float delta, const vector<vector<pair<float, int>>> &similar)
{
	if (sal1f.cols < 2)
		return;
...
void CmSaliencyRC::SmoothByHist(const Mat &img3f, Mat &sal1f, float delta)
{
	...
	int colorNums[3] = {12, 12, 12};
	int binN = Quantize(img3f, idx1i, binColor3f, colorNums1i, colorNums);
...
void CmSaliencyRC::SmoothByRegion(Mat &sal1f, const Mat &segIdx1i, int regNum, bool bNormalize)
{

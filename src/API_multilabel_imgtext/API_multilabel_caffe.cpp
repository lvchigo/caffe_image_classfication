...
/***********************************Init*************************************/
/// construct function 
API_MUTILABEL_CAFFE::API_MUTILABEL_CAFFE()
{
}

/// destruct function 
API_MUTILABEL_CAFFE::~API_MUTILABEL_CAFFE(void)
{
}

/***********************************Init*************************************/
int API_MUTILABEL_CAFFE::Init( 
...
int API_MUTILABEL_CAFFE::ReadImageToBlob( const vector<Mat_<Vec3f> > img_dl, int ModelMode, Blob<float>& image_blob)
{
	if( 0 == img_dl.size() )
		return 1;
...
bool API_MUTILABEL_CAFFE::DataMaxMin( const Dtype* d_from, int d_count, float& d_max, float& d_min )
{
  d_max = NMIN;
  d_min = NMAX;
...
void API_MUTILABEL_CAFFE::blob2image(Blob<float>& image_blob, UInt64 imgID){
...
float API_MUTILABEL_CAFFE::CalcL1Norm(const float* d_from, int d_count) 
{  
  double sum = 0.0;
  for (int i = 0; i < d_count; ++i) {
    sum += fabs(d_from[i]);
...
void API_MUTILABEL_CAFFE::info(Blob<float>& image_blob)
{
...
int API_MUTILABEL_CAFFE::DL_FileList2LabelFeat( int ModelMode, const vector<Mat_<Vec3f> > img_dl, UInt64 ImageID,
		const long labelNum, const char* layerName, vector< pair< int, float > > &vecLabel, vector< vector<float> > &imgFeat)		
{	
	int i,j,k,getLabelNum,batch_size,dim_features,dim_prob,nRet=0;
	long netLayerSize = 0;
	float iter_loss = 0.0;
	...
	caffe::shared_ptr<caffe::Blob<float> > prob_blob;	
	
	feature_blob = net_dl->blob_by_name(strLayerName);
	prob_blob = net_dl->blob_by_name("prob");
	...
	dim_prob = prob_blob->count() / batch_size;
	...
	for (i = 0; i < batch_size; ++i) {
		...
		//label
		prob_blob_data = prob_blob->cpu_data() + prob_blob->offset(i);
		for (j = 0; j < dim_prob; ++j) {
			vecRes.push_back( std::make_pair( j, prob_blob_data[j] ) );
		}
...
/***********************************Process**********************************/
int API_MUTILABEL_CAFFE::GetLabelFeat(
...
/***********************************Release**********************************/
void API_MUTILABEL_CAFFE::Release()
{
	if (net_dl) {
    		net_dl.reset();
  	}
